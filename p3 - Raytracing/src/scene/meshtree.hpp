...
public:
	MeshTreeNode();
	MeshTreeNode(const Mesh* mesh, std::vector<MeshTriangle> triangles);
	~MeshTreeNode();
	...
	Bound* createBBox(const Mesh* mesh);
	// get a bounding box from triangle
	Bound createTriangleBound(const Mesh* mesh, MeshTriangle triangle);
};
